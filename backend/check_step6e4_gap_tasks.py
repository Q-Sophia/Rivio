...
def main() -> None:
    ...
    needs = [
        InformationNeed(
            id=f"need_{dimension}",
            task_id=TASK_ID,
            question_id=f"kiq_{dimension}",
            dimension=dimension,
            required_facts=[f"当前 {dimension} 事实"],
            comparability_basis=f"统一 {dimension} 口径",
            decision_link="比较腾讯会议与 Zoom 的功能和价格",
        )
        for dimension in ("feature", "pricing")
    ]
    initial_tasks = [
        ResearchTask(
            id=f"researchtask_{name}_{dimension}_round_1",
            task_id=TASK_ID,
            information_need_id=f"need_{dimension}",
            title=f"采集 {competitor} {dimension} 资料",
            objective=f"获得 {dimension} 直接证据",
    ...
    require(
        all(
            item.information_need_id in {need.id for need in needs}
            for item in tasks
        ),
        "补采任务引用了 synthetic dangling InformationNeed",
    )
    ...
    require(second["new_research_task_count"] == 0, "最大轮次后仍新增补采任务")
